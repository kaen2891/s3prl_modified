...
class TransformerConfig(object):
    ...
    def __init__(self, config):
        ...
        self.pool = str(config['pool_type'])
...
class TransformerInputRepresentations(nn.Module):
    ...
    def __init__(self, config, input_dim):
        ...
        self.hidden_size = config.hidden_size
        self.spec_transform = nn.Linear(input_dim, config.hidden_size[0])

        # self.LayerNorm is not snake-cased to stick with TensorFlow model variable name and be able to load
        # any TensorFlow checkpoint file
        self.LayerNorm = TransformerLayerNorm(config.hidden_size[0], eps=config.layer_norm_eps)
...
class TransformerSelfAttention(nn.Module):
    def __init__(self, config, output_attentions=False, keep_multihead_output=False, th_layer=None):
        super(TransformerSelfAttention, self).__init__()
        if config.hidden_size[th_layer] % config.num_attention_heads != 0:
            raise ValueError(
        ...
        self.attention_head_size = int(config.hidden_size[th_layer] / config.num_attention_heads)
        self.all_head_size = self.num_attention_heads * self.attention_head_size

        self.query = nn.Linear(config.hidden_size[th_layer], self.all_head_size)
        self.key = nn.Linear(config.hidden_size[th_layer], self.all_head_size)
        self.value = nn.Linear(config.hidden_size[th_layer], self.all_head_size)
...
class TransformerSelfOutput(nn.Module):
    def __init__(self, config, th_layer):
        super(TransformerSelfOutput, self).__init__()
        self.pre_layer_norm = config.pre_layer_norm
        self.dense = nn.Linear(config.hidden_size[th_layer], config.hidden_size[th_layer])
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.LayerNorm = TransformerLayerNorm(config.hidden_size[th_layer], eps=config.layer_norm_eps)
...
class TransformerAttention(nn.Module):
    def __init__(self, config, output_attentions=False, keep_multihead_output=False, th_layer=None):
        ...
        self.self = TransformerSelfAttention(config, output_attentions=output_attentions,
                                              keep_multihead_output=keep_multihead_output, th_layer=th_layer)
        self.output = TransformerSelfOutput(config, th_layer)
...
class TransformerIntermediate(nn.Module):
    def __init__(self, config, th_layer):
        ...
        self.dense = nn.Linear(config.hidden_size[th_layer], config.intermediate_size[th_layer])
...
class TransformerOutput(nn.Module):
    def __init__(self, config, th_layer):
        ...
        self.pre_layer_norm = config.pre_layer_norm
        self.dense = nn.Linear(config.intermediate_size[th_layer], config.hidden_size[th_layer])
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.LayerNorm = TransformerLayerNorm(config.hidden_size[th_layer], eps=config.layer_norm_eps) # layer_norm for FFN
...
class TransformerLayer(nn.Module):
    def __init__(self, config, output_attentions=False, keep_multihead_output=False, th_layer=None):
        ...
        self.attention = TransformerAttention(config, output_attentions=output_attentions,
                                               keep_multihead_output=keep_multihead_output, th_layer=th_layer)
        self.intermediate = TransformerIntermediate(config, th_layer)
        self.output = TransformerOutput(config, th_layer)
        if self.pre_layer_norm:
            self.LayerNorm = self.output.LayerNorm
...
class TransformerEncoder(nn.Module):
    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        ...
        self.output_attentions = output_attentions
        self.pre_layer_norm = config.pre_layer_norm
        layer0 = TransformerLayer(config, output_attentions=output_attentions, keep_multihead_output=keep_multihead_output, th_layer=0)
        layer1 = TransformerLayer(config, output_attentions=output_attentions, keep_multihead_output=keep_multihead_output, th_layer=1)
        layer2 = TransformerLayer(config, output_attentions=output_attentions, keep_multihead_output=keep_multihead_output, th_layer=2)
        layer3 = TransformerLayer(config, output_attentions=output_attentions, keep_multihead_output=keep_multihead_output, th_layer=3)
        layer4 = TransformerLayer(config, output_attentions=output_attentions, keep_multihead_output=keep_multihead_output, th_layer=4)
        
        self.config = config
        
        if config.pool == 'max':
            self.pool_layer = nn.MaxPool2d((1, 2), stride=(1, 2))
        elif config.pool == 'avg':
            self.pool_layer = nn.AvgPool2d((1, 2), stride=(1, 2))
        elif config.pool == 'adap_max':
            self.pool_layer0 = nn.AdaptiveMaxPool2d((None, config.hidden_size[1]))
            self.pool_layer1 = nn.AdaptiveMaxPool2d((None, config.hidden_size[2]))
            self.pool_layer2 = nn.AdaptiveMaxPool2d((None, config.hidden_size[3]))
            self.pool_layer3 = nn.AdaptiveMaxPool2d((None, config.hidden_size[4]))
        
        
        if config.share_layer:
            #self.layer = nn.ModuleList([layer for _ in range(config.num_hidden_layers)])
            #self.layer = nn.ModuleList([layer0, layer1, layer2, layer3])
            self.layer = nn.ModuleList([layer0, layer1, layer2, layer3, layer4])
        else:
        ...
        if self.pre_layer_norm:
            ...
            LayerNorm0 = TransformerLayerNorm(config.hidden_size[0], eps=config.layer_norm_eps)
            LayerNorm1 = TransformerLayerNorm(config.hidden_size[1], eps=config.layer_norm_eps)
            LayerNorm2 = TransformerLayerNorm(config.hidden_size[2], eps=config.layer_norm_eps)
            LayerNorm3 = TransformerLayerNorm(config.hidden_size[3], eps=config.layer_norm_eps)
            LayerNorm4 = TransformerLayerNorm(config.hidden_size[4], eps=config.layer_norm_eps)
                        
            #self.LayerNorm = nn.ModuleList([LayerNorm0, LayerNorm1, LayerNorm2, LayerNorm3])
            self.LayerNorm = nn.ModuleList([LayerNorm0, LayerNorm1, LayerNorm2, LayerNorm3, LayerNorm4])
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, head_mask=None):
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if i < int(len(self.layer)-1):
                if self.config.pool != 'adap_max':
                    hidden_states = self.pool_layer(hidden_states.unsqueeze(1)).squeeze(1)
                else:
                    if i == 0:
                        hidden_states = self.pool_layer0(hidden_states.unsqueeze(1)).squeeze(1)
                    elif i == 1:
                        hidden_states = self.pool_layer1(hidden_states.unsqueeze(1)).squeeze(1)
                    elif i == 2:
                        hidden_states = self.pool_layer2(hidden_states.unsqueeze(1)).squeeze(1)
                    elif i == 3:
                        hidden_states = self.pool_layer3(hidden_states.unsqueeze(1)).squeeze(1)                        
                    else:
                        pass
...
class TransformerSpecPredictionHead(nn.Module):
    def __init__(self, config, output_dim, input_dim=None):
        ...
        if input_dim is None:
            ...
            self.dense = nn.Linear(input_dim, config.hidden_size[-1])
        if isinstance(config.hidden_act, str) or (sys.version_info[0] == 2 and isinstance(config.hidden_act, unicode)):
            self.transform_act_fn = ACT2FN[config.hidden_act]
        else:
            self.transform_act_fn = config.hidden_act
        self.LayerNorm = TransformerLayerNorm(config.hidden_size[-1], eps=config.layer_norm_eps)
        self.output = nn.Linear(config.hidden_size[-1], self.output_dim)
